...
# Create an empty dictionary for the arena 11 by 15
arena = dict()
for y in range(0, 11):
    arena[y] = dict()
    for x in range(0, 15):
...
# Return 'start' or 'none' depending on whether started.txt contains the character 1 or not
...
@basestation.route('/getArduinoData')
def getArduinoData():
    with open('arduino_data.txt', 'r') as infile:
        arduinoData = json.load(infile)

    return jsonify(arduinoData)


@basestation.route('/getMazeLayout')
def getMazeLayout():
    with open('maze_layout.txt', 'r') as infile:
        mazeLayout = json.load(infile)

    return jsonify(mazeLayout)
